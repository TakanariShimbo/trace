...
- 右下の操作リモコン（傾き・回転・パン十字・ホーム・ズーム）でも操作可。表示/非表示はサイドバーで切替
- **サイドバー**（左上 ☰ で開閉）に集約: 検索 / 地図 / 表示（リモコン切替）/ 事前保存
  - **検索**: 「山名／土地名／両方」を切替→候補タップでその地点へフライト
    - 山名: 山岳データ(`public/data/mountains.json`、約1,061山)を名前・読みで部分一致（標高・都道府県付き・オフライン可）
    - 土地名: 国土地理院 地名検索API ／ 両方: 山→土地名の順でマージ
